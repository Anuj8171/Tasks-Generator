...
export async function POST(request: Request) {
  try {
    ...
    const llm = getLLM(); // safe runtime initialization
...
`;
    ...
    if (text === "INVALID_INPUT") {
      return NextResponse.json({ error: "Input does not describe a meaningful feature idea." }, { status: 400 });
    }

    const spec = await prisma.spec.create({
      data: { goal, users, constraints, stories: text, tasks: text },
    });
